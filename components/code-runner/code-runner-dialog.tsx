...
export function CodeRunnerDialog({
    ...
    console.log("sampleRequest", typeof sampleRequest, sampleRequest);
    ...
    React.useEffect(() => {
        const next = typeof sampleRequest === "string"
            ? sampleRequest
            : sampleRequest != null
                ? JSON.stringify(sampleRequest, null, 2)
                : "";
        ...
        (payload: {
            ...
            buildRunBody
                ...
                : {
                    ...
                    content: payload.content, // edited (unsaved) code runs here
    ...
    const normalizeResult = (res: RunApiShape) => {
        if ("output" in res || "logs" in res) return { output: (res as any).output, logs: (res as any).logs ?? [] };
    ...
    const handleRun = async () => {
        ...
        setIsRunning(true);
        setTab("logs");
        setLogs((l) => [...l, { level: "info", message: "Starting run…", ts: new Date().toISOString() }]);
        ...
        try {
            ...
            setTab("logs");
            toast({ title: "Run failed", description: err?.message ?? "Error while running.", variant: "destructive" });
        } finally {
            setIsRunning(false);
        }
    };
                ...
                <div className="px-5 pt-5 pb-3 bg-background/80 backdrop-blur sticky top-0 z-20">
                    <DialogHeader className="flex flex-row items-center justify-between p-0">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            >
                                {stdinOpen ? <ChevronUp className="w-4 h-4 mr-1" /> : <ChevronDown className="w-4 h-4 mr-1" />} Input
                            </Button>
                            <Button onClick={handleRun} disabled={!current || isRunning} className="min-w-[124px]">
                            ...
                            <ResizablePanel defaultSize={70} minSize={40}>
                                <div className="h-full flex flex-col">
                                    ...
                                    <div className="flex items-center justify-between min-h-[44px] px-3 py-2 border-b bg-muted/30">
                                        ...
                                        <div className="flex items-center gap-2">
                                            {!editing ? (
                                                <Button size="sm" variant="outline" className="cursor-pointer" onClick={() => toggleEdit(true)}>
                                                ...
                                                <>
                                                    ...
                                                    <Button size="sm" variant="ghost" className="cursor-pointer" onClick={() => toggleEdit(false)}>
                                    ...
                                    <div className="flex-1 min-h-0">
                                        {current ? (
                                            <ArtifactCodeViewer
                                                ...
                                                readOnly={!editing}
                                                onChange={(val) => setDrafts((d) => ({ ...d, [current.artifact_id]: val }))}
                                            />
                                        ) : (
